<?php
/**
 * villagelanephotography functions and definitions
 *
 * @package villagelanephotography
 */

if ( ! function_exists( 'villagelanephotography_setup' ) ) :
/**
 ...
 */
function villagelanephotography_setup() {
	/*
	 * Make theme available for translation.
	 * Translations can be filed in the /languages/ directory.
	 * If you're building a theme based on villagelanephotography, use a find and replace
	 * to change 'villagelanephotography' to the name of your theme in all the template files
	 */
	load_theme_textdomain( 'villagelanephotography', get_template_directory() . '/languages' );
...
endif; // villagelanephotography_setup
add_action( 'after_setup_theme', 'villagelanephotography_setup' );
 ...
 */
function villagelanephotography_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'villagelanephotography_content_width', 640 );
}
add_action( 'after_setup_theme', 'villagelanephotography_content_width', 0 );
 ...
 */
function villagelanephotography_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'villagelanephotography' ),
		'id'            => 'sidebar-1',
		'description'   => '',
...
add_action( 'widgets_init', 'villagelanephotography_widgets_init' );


/**
 * Add WooCommerce support to theme
 */
function vl_add_woocommerce_support() {
	add_theme_support( 'woocommerce' );
}
add_action( 'after_setup_theme', 'vl_add_woocommerce_support' );

/**
 * Enqueue scripts and styles.
 */
function villagelanephotography_scripts() {
	...
	wp_enqueue_style( 'villagelanephotography-style', get_stylesheet_uri(), array(), filemtime( get_stylesheet_directory() . '/style.css' ) );
	...
	if( is_page() ){ //Check if we are viewing a page
		...
		if($template_name == 'page-home.php'){
			wp_enqueue_script( 'villagelanephotography-home-scripts', get_template_directory_uri() . '/js/scripts-home-min.js', array('jquery', 'flickity-script'), filemtime( get_template_directory() . '/js/scripts-home-min.js' ), true );
		}
	}

	wp_enqueue_script( 'villagelanephotography-scripts', get_template_directory_uri() . '/js/scripts-min.js', array( 'jquery', 'images-loaded' ), filemtime( get_template_directory() . '/js/scripts-min.js' ), true );

	wp_enqueue_script( 'villagelanephotography-navigation', get_template_directory_uri() . '/js/navigation-min.js', array(), filemtime( get_template_directory() . '/js/navigation-min.js' ), true );

	wp_enqueue_script( 'villagelanephotography-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), filemtime( get_template_directory() . '/js/skip-link-focus-fix.js' ), true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'villagelanephotography_scripts' );
...
function vl_menucart($menu, $args) {
	...
	ob_start();
		global $woocommerce;
		$viewing_cart = __('View your shopping cart', 'villagelanephotography');
		$start_shopping = __('Start shopping', 'villagelanephotography');
...
add_action( 'wp_head', 'vl_svg_sprites' );